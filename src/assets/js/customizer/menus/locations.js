				...
				if ( isExpanded ) {
					_.each( $( '[id^="customize-control-nav_menu_locations-"]' ), function( menuLocation ) {
						...
						if ( ! $this.find( '.edit-menu-design' ).length ) {
							...
							html = `<a href="javascript:wp.customize.panel( 'bgtfw_menu_location_${location}' ).expand();" type="button" class="button-link edit-menu-design" aria-label="Edit Menu Design">Edit Menu Design</a>`;
