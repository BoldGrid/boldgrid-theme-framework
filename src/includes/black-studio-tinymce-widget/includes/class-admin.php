	...
	final class Black_Studio_TinyMCE_Admin {
		...
		public function admin_init() {
			...
			if ( $this->enabled() ) {
				...
				// BoldGrid: Remove Links to BS.  add_action( 'black_studio_tinymce_before_editor', array( $this, 'display_links' ) ); // consider donating if you remove links
		...
		public function enqueue_style() {
			...
			wp_enqueue_style(
				$style,
				boldgrid_plugin_dir( 'css/' . $style . $suffix. '.css', dirname( __FILE__ ) ),
				array(),
				bstw()->get_version()
			);
		}
		...
		public function enqueue_script() {
			...
			wp_enqueue_script(
				$script,
				boldgrid_plugin_dir( 'js/' . $script . $suffix . '.js', dirname( __FILE__ ) ),
		...
		public function wp_tiny_mce_init() {
			...
			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			echo "\t\t" . '<script type="text/javascript" src="' . boldgrid_plugin_dir( 'js/' . $script . $suffix . '.js', dirname( __FILE__ ) ) . '"></script>' . "\n"; // xss ok
		}
